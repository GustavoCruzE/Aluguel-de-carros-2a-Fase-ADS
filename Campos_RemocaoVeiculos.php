...
$sql = "SELECT * FROM InformacoesVeiculos";
$veiculos = $mysqli->query($sql);
?>
...
<head>
    ...
    <title>Remoção de veículos</title>
    <style>
        body {
...
<body>
    ...
    <table>
        ...
        <?php while ($row = mysqli_fetch_assoc($veiculos)) : ?>
            <tr>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $Placa = $_POST['Placa'];
        $sqlDelete = "DELETE FROM InformacoesVeiculos WHERE Placa = '$Placa'";
        $resultado = mysqli_query($mysqli, $sqlDelete);
        if ($resultado) {
            echo "<p>Exclusão bem sucedida.</p>";
        } else {
            echo "<p>Ocorreu um erro ao excluir o veículo.</p>";
        }
        mysqli_close($mysqli);
    }
    ?>
